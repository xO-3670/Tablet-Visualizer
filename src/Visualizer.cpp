...
{   
    while (_Window.isOpen())
    {
        ...
        _Cursor.Update(sf::Vector2f(// Get mouse position // make it move only in tablet area // Add offset so it shows that cursor moves in tablet active area + offset from users's area
            sf::Mouse::getPosition().x * (_TabletPlayfield.width  / static_cast<float>(_DesktopDimensions.x)) + _TabletImageOffset.x + _TabletPlayfield.left,
            sf::Mouse::getPosition().y * (_TabletPlayfield.height / static_cast<float>(_DesktopDimensions.y)) + _TabletImageOffset.y + _TabletPlayfield.top
        ));
